...
* По-умолчанию показываются только новые комментарии
* Комментарии должны быть отсортированы от более старых к более новым
* Должна быть реализована настраиваемая пагинация
...
QUERY PARAMETERS
* __filter__ (string, Default: "new") - фильтр комментариев по статусу (new, rejected, published)
* __from__ (integer, >= 0, Default: 0) - количество элементов, которые нужно пропустить для формирования текущего набора
* __size__ (integer, Default: 10) - количество элементов в наборе
...
Удаление комментария пользователя в любом статусе
